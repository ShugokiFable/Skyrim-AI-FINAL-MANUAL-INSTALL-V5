---
name: skyrim-memory
description: Consult the Codex / GPT Skyrim evidence registry before substantial mod work so previously observed failures
  are not repeated.
metadata:
  version: 5.0.0
  provider: codex
...
- `provider-specific` entries apply only to Codex.
