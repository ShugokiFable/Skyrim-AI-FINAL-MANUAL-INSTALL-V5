---
name: skyrim-memory
description: Consult the Grok Build Skyrim evidence registry before substantial mod work so previously observed failures are
  not repeated.
metadata:
  version: 5.0.0
  provider: grok
...
- `provider-specific` entries apply only to Grok.
